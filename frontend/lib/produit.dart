...
class Produit {
  ...
  static getIdProduit(BuildContext context, id) async {
    try {
      print('id numero : ' + id);
      Navigator.pushNamed(context, '/modifier', arguments: id);
    } catch (err) {
      return Future.error(err);
    }
  }
  ...
  static modifier(BuildContext context, id) async {
    ...
    try {
      var res = await http.get(Uri.parse(
          'http://127.0.0.1:8000/api/modifierProduit/$id'));
      if (res.statusCode == 200) {
        Navigator.pushNamed(context, '/liste');
      } else {
        Navigator.pushNamed(context, '/');
